...
garanti: "garanti.pdf",
qnb: "qnb1.pdf",
// Yeni normal dekont referansları
ing: "ing-fast.pdf",
teb: "teb-havale.pdf",
kuveytturk: "kuveytturk-fast.pdf",
};
...
if (
value === "ing" ||
value.includes("ingbank") ||
value.includes("ingbankasi")
) {
return "ing"
}
...
if (
value === "teb" ||
value.includes("turkekonomibankasi") ||
value.includes("turkekonomi")
) {
return "teb"
}

if (
value.includes("kuveytturk") ||
value.includes("kuveytturkbank")
) {
return "kuveytturk"
}
...
function rfMetricDistance(a, b) {
  if (!a || !b) return null;
  const rel = (x, y, scale) => {
    const av = Number(x), bv = Number(y);
    if (!Number.isFinite(av) || !Number.isFinite(bv)) return null;
    return Math.min(1, Math.abs(av - bv) / Math.max(Number(scale) || 1e-6, 1e-6));
  };
  // Prefer local, scale-normalized ink/style measurements. Raw mean luminance
  // is intentionally excluded because PDF text and camera/JPEG backgrounds
  // have very different absolute luminance distributions.
  const parts = [
    rel(a.tightInkDarkRatio, b.tightInkDarkRatio, 0.16),
    rel(a.edgeRatio, b.edgeRatio, 0.10),
    rel(a.lumaStd, b.lumaStd, 28),
    rel(a.rowVariance, b.rowVariance, 0.022),
    rel(a.colVariance, b.colVariance, 0.022),
    rel(a.aspect, b.aspect, 0.35),
  ].filter(Number.isFinite);
...
async function rfCharacterMetrics(imageBuffer, region, imageSize) {
  ...
  try {
    ...
    // IMPORTANT: Never compare raw pixels from a PDF raster with raw pixels from
    // a camera/JPEG image. Both are first reduced to a common coordinate system,
    // then all character measurements are expressed relative to the local text
    // geometry. This makes the fingerprint much less sensitive to resolution,
    // crop and camera distance.
    const marginX = Math.max(1, Math.round(w * 0.06));
    const marginY = Math.max(1, Math.round(h * 0.22));
    const left = Math.max(0, x - marginX);
    const top = Math.max(0, y - marginY);
    const width = Math.min(imageSize.width - left, w + marginX * 2);
    const height = Math.min(imageSize.height - top, h + marginY * 2);
    ...
    const { data, info } = await sharp(imageBuffer)
      .extract({ left, top, width, height })
      .resize({ width: 240, height: 96, fit: 'fill' })
      .grayscale()
      .raw()
      .toBuffer({ resolveWithObject: true });
    ...
    // Connected components. Border/box artifacts are discarded before the
    ...
    for (let sy = 0; sy < H; sy++) {
      for (let sx = 0; sx < W; sx++) {
        ...
        // Very long components are usually borders/lines, not glyphs.
        if (area >= 3 && cw <= W * 0.45 && ch <= H * 0.72) {
          comps.push({ minX, maxX, minY, maxY, width: cw, height: ch, area });
        }
      }
    }

    const body = comps.filter(c => c.height >= 6 && c.area >= 8).sort((a,b) => b.area - a.area).slice(0, 60);
    const bodyHeights = body.map(c => c.height);
    const bodyWidths = body.map(c => c.width);
    const bodyAreas = body.map(c => c.area);
    ...
    // Tight ink box = robust density measurement independent of OCR-box padding.
    const ink = body.length ? {
      minX: Math.min(...body.map(c => c.minX)),
      maxX: Math.max(...body.map(c => c.maxX)),
      minY: Math.min(...body.map(c => c.minY)),
      maxY: Math.max(...body.map(c => c.maxY)),
    } : null;
    const inkW = ink ? Math.max(1, ink.maxX - ink.minX + 1) : 1;
    const inkH = ink ? Math.max(1, ink.maxY - ink.minY + 1) : 1;
    const tightInkArea = inkW * inkH;
    const tightInkDarkRatio = body.length ? body.reduce((s,c)=>s+c.area,0) / tightInkArea : 0;

    // Stroke proxy: average component fill ratio. It is not a font renderer;
    // it is a stable raster proxy for relative stroke weight.
    const fillRatios = body.map(c => c.area / Math.max(1, c.width * c.height));
    const medianFillRatio = rfMedian(fillRatios);

    // Horizontal rhythm normalized by local character height.
    const sortedX = body.map(c => (c.minX + c.maxX) / 2).sort((a,b)=>a-b);
    const gaps = [];
    for (let i = 1; i < sortedX.length; i++) {
      const g = sortedX[i] - sortedX[i - 1];
      if (g > 0 && g < Math.max(45, medianBodyHeight * 3.2)) gaps.push(g / Math.max(1, medianBodyHeight));
    }

    // Diacritic candidate = a small component above a nearby body component.
    // We require horizontal overlap so random JPEG noise above the text is not
    // counted as a diacritic.
    const diacriticCandidates = comps.filter(c => {
      if (c.height < 2 || c.height > Math.max(4, medianBodyHeight * 0.42)) return false;
      if (c.area > Math.max(10, medianBodyArea * 0.45)) return false;
      const hasBodyBelow = body.some(b => {
        const overlapX = Math.min(c.maxX, b.maxX) - Math.max(c.minX, b.minX);
        const verticalGap = b.minY - c.maxY;
        return overlapX >= -1 && verticalGap >= -Math.max(2, medianBodyHeight * 0.10) && verticalGap <= Math.max(18, medianBodyHeight * 0.9);
      });
      return hasBodyBelow && c.minY < H * 0.55;
    });
    const diaArea = diacriticCandidates.reduce((s,c)=>s+c.area,0);
    const diaFill = diacriticCandidates.length
      ? rfMedian(diacriticCandidates.map(c=>c.area / Math.max(1,c.width*c.height)))
      : 0;
    const diaHeightNorm = diacriticCandidates.length
      ? rfMedian(diacriticCandidates.map(c=>c.height / Math.max(1, medianBodyHeight)))
      : 0;
    const diaGapNorm = diacriticCandidates.length
      ? rfMedian(diacriticCandidates.map(c => {
          const nearest = body.filter(b => Math.min(c.maxX,b.maxX)-Math.max(c.minX,b.minX) >= -1)
            .sort((a,b)=>Math.abs(a.minY-c.maxY)-Math.abs(b.minY-c.maxY))[0];
          return nearest ? Math.max(0, nearest.minY-c.maxY)/Math.max(1,medianBodyHeight) : 0;
        }))
      : 0;
    ...
    return {
      ...
      medianCharacterAspect: medianBodyHeight ? medianBodyWidth / medianBodyHeight : 0,
      medianCharacterFillRatio: medianFillRatio,
      characterHeightNorm: medianBodyHeight / H,
      characterWidthNorm: medianBodyWidth / W,
      characterHeightSpreadNorm: medianBodyHeight ? (bodyHeights.length ? Math.sqrt(bodyHeights.reduce((s,v)=>s+(v-medianBodyHeight)**2,0)/bodyHeights.length)/medianBodyHeight : 0) : 0,
      characterWidthSpreadNorm: medianBodyWidth ? (bodyWidths.length ? Math.sqrt(bodyWidths.reduce((s,v)=>s+(v-medianBodyWidth)**2,0)/bodyWidths.length)/medianBodyWidth : 0) : 0,
      characterAreaSpreadNorm: medianBodyArea ? (bodyAreas.length ? Math.sqrt(bodyAreas.reduce((s,v)=>s+(v-medianBodyArea)**2,0)/bodyAreas.length)/medianBodyArea : 0) : 0,
      bodyHeightNorm: inkH / H,
...
function rfFocusLabelRegion(region, fullText) {
  if (!region) return null;
  const s = String(fullText || '');
  if (!/[:：]/.test(s)) return region;
  const left = String(s.split(/[:：]/)[0] || '').trim();
  const total = s.replace(/[:：]/g, '').trim();
  if (!left || !total) return region;
  const ratio = Math.max(0.12, Math.min(0.78, left.length / total.length));
  const x1 = Number(region.x1), x2 = Number(region.x2);
  const pad = Math.min(0.04, Math.max(0.01, ratio * 0.08));
  return {
    x1,
    y1: Number(region.y1),
    x2: x1 + (x2 - x1) * Math.min(0.82, ratio + pad),
    y2: Number(region.y2),
  };
}
...
function resolveVisualReferencePath(reference) {
  ...
  return candidates.find((p) => /\.(png|jpe?g|webp)$/i.test(String(p))) || null;
}

async function runReferenceForensicEngine(targetPath, bank, targetOCR, selectedReferencePath = null, visualReferencePath = null) {
  const normalizedBank = normalizeBank(bank);
  ...
  try {
    ...
    const referencePaths = selectedReferencePath && allReferencePaths.includes(selectedReferencePath)
      ...
      : allReferencePaths;
    if (!referencePaths.length) return null;
    const visualRefPath = visualReferencePath && /\.(png|jpe?g|webp)$/i.test(String(visualReferencePath)) ? visualReferencePath : null;
    ...
    for (const referencePath of referencePaths) {
      let refTemp=null;
      try {
        const structuralPath = referencePath;
        const sourcePath = visualRefPath || (/\.(png|jpe?g|webp)$/i.test(String(referencePath)) ? referencePath : structuralPath);
        ...
        const cacheKey=`rfocr39:${normalizedBank}:${path.basename(sourcePath)}:${refId}`;
        ...
        console.log('REFERENCE FORENSIC FIELD MATCHING:',JSON.stringify({
          bank:normalizedBank,reference:path.basename(sourcePath),structuralReference:path.basename(structuralPath),
        ...
        const tpKind=(text)=>{
          const s=String(text||'').trim();
          const hasDigit=/\d/.test(s), hasLetter=/[A-Za-zÇĞİÖŞÜçğıöşü]/.test(s);
        ...
        for(const m of matches){
          ...
          const profile={
            field:key,
            labelReference:refLabelText,
            labelTarget:tarLabelText,
            labelComparable:true,
            labelMatch:exactLabel?'exact':'near-exact',
            labelOCRConfidence:{reference:refConf,target:tarConf},
...
function isStatementReferencePath(filePath) {
  const base = path.basename(String(filePath || '')).toLocaleLowerCase('tr-TR');
  const knownStatementFiles = new Set(Object.values(STATEMENT_REFERENCE_MAP)
    .flatMap(v => Array.isArray(v) ? v : [v])
    .filter(Boolean)
    .map(v => String(v).toLocaleLowerCase('tr-TR')));
  if (knownStatementFiles.has(base)) return true;

  // Savunma katmanı: hesap özeti/hareket dosyaları normal dekont referansı
  // olarak hiçbir koşulda kullanılmamalı.
  return /hesap[-_ ]?(hareketleri|hareketi|ozeti|özeti)/i.test(base);
}
...
if (referenceAmountField) {
...
} else if (referenceTemplateAvailable) {
  // GÜVENLİ REGRESYON KORUMASI:
  // Bankaya ait güvenilir referans profili mevcut fakat referans amount
  ...
  if (primaryInlineAmountCandidates.length) {
    ...
    if (reconstructedLabelMatched.length) {
      candidatePool = reconstructedLabelMatched;
      selectionMethod = "reconstructed-amount-label-no-anchor";
    } else if (directLabelMatched.length) {
      candidatePool = directLabelMatched;
      selectionMethod = "direct-amount-label-no-anchor";
    } else if (labelMatched.length) {
      candidatePool = labelMatched;
...
Referans verilirse yalnızca görsel/teknik baseline olarak kullan:
- sayfa ölçüsü ve genel kompozisyon
- logo/başlık yerleşimi
- müşteri-hesap bilgi kutusu geometrisi
- işlem bilgi kutusu
- tablo kolonları, hizalama ve satır aralıkları
- font görünümü, ağırlığı ve boyutu
- çizgi, kutu, footer ve sayfa düzeni

REFERANSTAKİ müşteri adı, IBAN, hesap numarası, tarih, tutar, bakiye, açıklama veya diğer dinamik değerleri hedef belgenin doğru değeri olarak kabul etme.
...
const visualReferencePath = resolveVisualReferencePath(reference);
...
console.log("REFERENCE OCR SOURCE:", visualReferencePath ? path.basename(visualReferencePath) : (reference?.path ? path.basename(reference.path) : "YOK"));
...
async function runDirectReferenceDifferenceEngine({ targetPath, referenceInfo, targetOCR = null, bank = null }) {
  /*
   * V43 — FOCUSED REFERENCE COMPARISON
   *
   * V42 problem:
   *   - 18 images were sent in one giant visual request.
   *   - The model returned candidateCount=0 even though the deterministic
   *     typography engine had strong localized evidence.
   *   - The normalized zone coordinates were treated as if target/reference
   *     were perfectly aligned.
   *
   * V43:
   *   - Compare ONE semantic page zone at a time (exactly 2 images/request).
   *   - Ask the visual model to reason about small translation/scale differences
   *     inside the pair instead of assuming pixel-perfect alignment.
   *   - Keep the request focused on rendering differences, not OCR content.
   *   - Run the eight zone reviews concurrently.
   *   - Run a separate adjudication pass over the resulting candidates.
   *   - Preserve target-relative normalized boxes for the annotator.
   */
  try {
    if (!referenceInfo?.path || !targetPath) return null;
    ...
    // Overlapping zones deliberately cover the whole document.  They are not
    // semantic fields; they are visual inspection windows.
    const zones = [
      { id:"header",       label:"Üst bölüm",       x:0.00, y:0.00, w:1.00, h:0.25 },
      { id:"left_upper",   label:"Sol üst blok",   x:0.00, y:0.16, w:0.56, h:0.28 },
      { id:"right_upper",  label:"Sağ üst blok",   x:0.44, y:0.16, w:0.56, h:0.28 },
      { id:"left_middle",  label:"Sol orta blok",  x:0.00, y:0.34, w:0.56, h:0.30 },
      { id:"right_middle", label:"Sağ orta blok",  x:0.44, y:0.34, w:0.56, h:0.30 },
      { id:"left_lower",   label:"Sol alt blok",   x:0.00, y:0.54, w:0.56, h:0.27 },
      { id:"right_lower",  label:"Sağ alt blok",   x:0.44, y:0.54, w:0.56, h:0.27 },
      { id:"footer",       label:"Alt bölüm",      x:0.00, y:0.70, w:1.00, h:0.30 }
    ];

    const cropZone = async (buffer, meta, zone, width = 1800) => {
        ...
        findings:{
          ...
          items:{
            ...
            properties:{
              ...
              issueType:{
                type:"string",
                enum:["content","typography","layout","spacing","local-render","mixed"]
              },
              confidence:{type:"integer",minimum:0,maximum:100},
              evidence:{type:"string"},
              // Coordinates are relative to THIS zone.
              targetBox:{
                type:"object",
                properties:{
            ...
            required:["field","issueType","confidence","evidence","targetBox"],
            additionalProperties:false
          }
        }
      },
    ...
    const reviewZone = async (zone) => {
      try {
        const [referenceImage, targetImage] = await Promise.all([
          cropZone(referenceBuffer, referenceMeta, zone, 1800),
          cropZone(targetBuffer, targetMeta, zone, 1800)
        ]);

        const content = [
          {
            type:"input_text",
            text:`VERIFYDOC V43 — REFERANS BÖLGE KARŞILAŞTIRMASI
...
Bu istekte yalnızca ${zone.label} bölgesini inceliyorsun.

AMAÇ:
TARGET ile REFERENCE arasındaki GERÇEK belge üretim farklarını bulmak.
Bu bir OCR görevi değildir. OCR'daki yazı farklılığına değil, görüntüdeki
yazının/alanın nasıl üretildiğine bak.

ÖNCE iki görüntüde aynı belge bölgesini zihinsel olarak hizala.
Birkaç piksel yatay/dikey kayma veya küçük ölçek farkı NORMALDİR.
Bunları fark olarak raporlama.

ÖZELLİKLE ARA:
- aynı alanın fontunun değişmesi
- harf/rakam şeklinin değişmesi
- aynı sayı içinde tek tek rakamların farklı font/stroke üretimi
- yazının kalınlığı/koyuluğu/silikliği
- karakter genişliği ve aralığı
- baseline ve satır hizası
- aynı bloktaki değerlerin diğer değerlerden farklı raster üretilmesi
- bir alanın eklenmiş, silinmiş veya yeniden yazılmış görünmesi
- gerçek layout/spacing değişikliği
- lokal yapıştırma/yeniden üretim izleri

ŞUNLARI RAPORLAMA:
- fotoğraf perspektifi
- ışık/gölge
- ekran yansıması
- JPEG sıkıştırması
- çözünürlük farkı
- hafif blur/anti-aliasing
- tüm sayfaya yayılan küçük kalite farkları
- Türkçe karakterlerin tek başına raster farkı
- yalnızca değer/metin farklı diye content farkı

ÇOK ÖNEMLİ:
Bir alanın içeriği farklı olsa bile bu tek başına sahtecilik bulgusu değildir.
Ancak o alanın TARGET'taki görsel üretimi REFERENCE'tan açıkça farklıysa
typography/local-render/layout bulgusu üret.

Aynı büyük değişikliği birkaç ayrı bulguya bölme.
Örneğin bir sol blokta birkaç değer aynı farklı fontla üretilmişse bunu
tek güçlü bulguda "Sol işlem bilgileri / değer alanları" olarak birleştir.

Bulgu yoksa findings boş olsun.
Tahmin etme.

targetBox yalnızca farklı görünen TARGET alanını kapsasın.
Koordinatlar bu zone görüntüsüne göre 0..1 arasındadır.`
          },
        ...
        const response = await openai.responses.create({
          ...
          text:{
            format:{
              type:"json_schema",
              name:`v43_zone_${zone.id.replace(/[^a-z0-9_]/gi,"_")}`,
              strict:true,
              schema
            }
          }
        ...
        console.warn(`V43 ZONE ${zone.id} HATASI:`, error?.message || error);
        return {zone, findings:[]};
      }
    };

    // Eight small visual comparisons are much more reliable than one huge
    // 18-image attention-heavy request.
    const zoneResults = await Promise.all(zones.map(reviewZone));

    const candidates = [];
    for (const zr of zoneResults) {
      const z = zr.zone;
      for (const f of zr.findings) {
        const b = f?.targetBox;
        const lx = Number(b?.x), ly = Number(b?.y);
        const lw = Number(b?.w), lh = Number(b?.h);

        if (![lx,ly,lw,lh].every(Number.isFinite)) continue;
        if (lw <= 0 || lh <= 0 || lx < 0 || ly < 0 || lx + lw > 1.001 || ly + lh > 1.001) continue;

        const confidence = Number(f.confidence) || 0;
        const evidence = String(f.evidence || "").trim();

        // Avoid weak visual guesses.  78 is intentionally lower than the
        // final verification threshold because the second pass decides.
        if (confidence < 78 || evidence.length < 20) continue;

        const gx = z.x + lx * z.w;
        const gy = z.y + ly * z.h;
        const gw = Math.min(lw * z.w, 1 - gx);
        const gh = Math.min(lh * z.h, 1 - gy);

        candidates.push({
          zoneId:z.id,
          zoneLabel:z.label,
          field:String(f.field || "Belge bölümü"),
          issueType:String(f.issueType || "mixed"),
          confidence,
          evidence,
          targetBox:{
            x:Math.max(0,Math.min(1,gx)),
            y:Math.max(0,Math.min(1,gy)),
            w:Math.max(0,Math.min(gw,1)),
            h:Math.max(0,Math.min(gh,1))
          }
        });
      }
    }

    // Deduplicate overlapping candidates before verification.
    candidates.sort((a,b)=>b.confidence-a.confidence);
    const uniqueCandidates = [];
    for (const c of candidates) {
      const b=c.targetBox;
      const duplicate=uniqueCandidates.some(d=>{
        const db=d.targetBox;
        const ix=Math.max(
          0,
          Math.min(b.x+b.w,db.x+db.w)-Math.max(b.x,db.x)
        );
        const iy=Math.max(
          0,
          Math.min(b.y+b.h,db.y+db.h)-Math.max(b.y,db.y)
        );
        const inter=ix*iy;
        const minArea=Math.min(b.w*b.h,db.w*db.h);
        const sameCenter=
          Math.abs((b.x+b.w/2)-(db.x+db.w/2))<0.025 &&
          Math.abs((b.y+b.h/2)-(db.y+db.h/2))<0.020;
        return (minArea>0 && inter/minArea>0.50) || sameCenter;
      });
      if (!duplicate) uniqueCandidates.push(c);
      if (uniqueCandidates.length >= 18) break;
    }
    ...
    // Independent verification.  Each candidate gets a wider context window.
    // The verifier is explicitly forbidden from accepting a difference caused
    // only by alignment, photography or compression.
    const verifySchema = {
      type:"object",
      properties:{
        results:{
          ...
          items:{
            type:"object",
            properties:{
              candidateIndex:{type:"integer",minimum:1,maximum:18},
              confirm:{type:"boolean"},
              confidence:{type:"integer",minimum:0,maximum:100},
              reason:{type:"string"}
            },
            required:["candidateIndex","confirm","confidence","reason"],
            additionalProperties:false
          }
        }
      },
    ...
    const verifyTasks = uniqueCandidates.map(async (c, index) => {
      try {
        const padX=Math.min(0.10,Math.max(0.035,c.targetBox.w*1.25));
        const padY=Math.min(0.09,Math.max(0.045,c.targetBox.h*1.8));

        const cx=Math.max(0,c.targetBox.x-padX);
        const cy=Math.max(0,c.targetBox.y-padY);
        const cw=Math.min(1-cx,c.targetBox.w+2*padX);
        const ch=Math.min(1-cy,c.targetBox.h+2*padY);

        const contextZone={x:cx,y:cy,w:cw,h:ch};
        const [referenceContext,targetContext]=await Promise.all([
          cropZone(referenceBuffer,referenceMeta,contextZone,1500),
          cropZone(targetBuffer,targetMeta,contextZone,1500)
        ]);

        return [
          {type:"input_text",text:`ADAY ${index+1}
Alan: ${c.field}
İlk bulgu: ${c.evidence}

Bu adayın gerçek bir referans farkı olup olmadığını bağımsız olarak kontrol et.
TARGET ve REFERENCE aynı belgeye ait karşılık gelen bölge olarak düşün.
Küçük kaymaları zihinsel olarak tolere et.

ONAYLA ancak:
- lokal bir görsel üretim farkı açıkça görülüyorsa,
- fark font/rakam geometrisi/stroke/koyuluk/spacing/layout/yeniden üretim gibi
  belgeye özgü bir nedene dayanıyorsa.

REDDET:
- yalnızca fotoğraf açısı/ışık/blur/JPEG/ölçek,
- yalnızca OCR veya metin içeriği farkı,
- tüm sayfaya yayılmış kalite farkı,
- Türkçe diakritik farkı,
- belirsiz veya gözle doğrulanamayan fark.

Bu aday gerçekten görünür mü?`},
          {type:"input_text",text:"REFERENCE BAĞLAM:"},
          {type:"input_image",image_url:referenceContext,detail:"high"},
          {type:"input_text",text:"TARGET BAĞLAM:"},
          {type:"input_image",image_url:targetContext,detail:"high"}
        ];
      } catch (error) {
        return null;
      }
    ...
    const verifyInputs = (await Promise.all(verifyTasks)).filter(Boolean);

    const verified = [];
    // Verify in batches of four to keep each request visually focused.
    for (let offset=0; offset<verifyInputs.length; offset+=4) {
      const batch = verifyInputs.slice(offset,offset+4);
      const content=[
        {
          type:"input_text",
          text:`VERIFYDOC V43 — BAĞIMSIZ REFERANS FARKI DOĞRULAMA

Aşağıda ilk taramada bulunan adaylar var.
Her adayın TARGET ve REFERENCE görüntüsünü karşılaştır.

Amaç, SADECE gerçek ve lokal referans farklarını bırakmak.
Bir aday fotoğraf kaynaklıysa veya yalnızca içerik farklıysa confirm=false.

Aynı görsel farkı tekrar eden adayları onaylama.

Sonuçları aday numarasıyla döndür.`
        }
      ];
      ...
      try {
        const vr=await openai.responses.create({
          ...
          text:{
            format:{
              type:"json_schema",
              name:`v43_verify_batch_${Math.floor(offset/4)+1}`,
              strict:true,
              schema:verifySchema
            }
          }
        ...
        const parsed=parseAIResponse(vr?.output_text || "");
        for (const r of (Array.isArray(parsed?.results)?parsed.results:[])) {
          const localIndex=Number(r?.candidateIndex)-1;
          const globalIndex=offset+localIndex;
          const c=uniqueCandidates[globalIndex];
          if (!c) continue;
          if (r?.confirm!==true) continue;
          if (Number(r?.confidence)<84) continue;
        ...
        console.warn("V43 DOĞRULAMA BATCH HATASI:",error?.message || error);
      }
    }
    ...
    for (const f of verified.sort((a,b)=>b.confidence-a.confidence)) {
      const b=f.targetBox;
      const duplicate=finalFindings.some(d=>{
        const db=d.targetBox;
        const ix=Math.max(
          0,
          Math.min(b.x+b.w,db.x+db.w)-Math.max(b.x,db.x)
        ...
        const inter=ix*iy;
        const minArea=Math.min(b.w*b.h,db.w*db.h);
        return (minArea>0 && inter/minArea>0.45);
      });
      if (!duplicate) finalFindings.push(f);
    }
    ...
    return {
      ...
      findings:finalFindings.slice(0,12),
      zonesChecked:zones.map(z=>z.id),
      candidateCount:uniqueCandidates.length,
      verifiedCount:finalFindings.length
    };
  } catch (error) {
    console.warn("V43 FOCUSED REFERENCE DIFFERENCE HATASI:",error?.message || error);
    return null;
  }
}
...
if ((type === 'image' || type === 'pdf') && bank && reference && shouldRunTerra) {
  try {
    referenceVisualAdjudication = await runDirectReferenceDifferenceEngine({targetPath:forensicTargetPath,referenceInfo:(visualReferencePath ? {...reference, path:visualReferencePath, fileName:path.basename(visualReferencePath)} : reference),targetOCR:paddleImageOCR,bank});
...
if ((type === "image" || type === "pdf") && bank && reference) {
  try {
    const trustedReferencePaths = visualReferencePath ? [visualReferencePath] : (reference?.path ? [reference.path] : []);
...
async function runReferenceLocalCropComparator(targetPath, bank, targetOCR, selectedReferencePath = null) {
  ...
  try {
    ...
    const referencePath = selectedReferencePath && /\.(png|jpe?g|webp)$/i.test(String(selectedReferencePath))
      ? selectedReferencePath
      : (referencePaths.find(p => /\.(png|jpe?g|webp)$/i.test(String(p))) || referencePaths.find(p => /\.(pdf)$/i.test(String(p))));
...
if (visualForensics?.available && visualForensics?.severity === "strong") {
  ...
  // Keep this signal for deterministic risk; do not expose raw engine wording
...
if (layoutForensics?.available && layoutForensics.severity === "strong") {
  // Layout signal remains available to the deterministic risk engine only.
}

if (referenceForensics?.available && referenceForensics.severity !== "low") {
  // Reference engine evidence is kept internally; Telegram uses only the
  // clean reference comparison report generated above.
}
